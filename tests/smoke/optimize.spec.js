...
test.describe.serial("optimize", () => {
    ...
    test("re-optimizes asset when source fingerprint changes", () => {
        const cacheBefore = JSON.parse(readFileSync(CACHE_PATH, "utf8"));
        const [assetKey, assetBefore] = Object.entries(cacheBefore.images)[0];

        const staleCache = JSON.parse(readFileSync(CACHE_PATH, "utf8"));
        staleCache.images[assetKey].sourceFingerprint = "0".repeat(64);
        writeFileSync(CACHE_PATH, JSON.stringify(staleCache, null, 2) + "\n");
        ...
        try {
            ...
            const cacheAfter = JSON.parse(readFileSync(CACHE_PATH, "utf8"));
            const assetAfter = cacheAfter.images[assetKey];

            expect(assetAfter.sourceFingerprint).not.toBe("0".repeat(64));
            expect(assetAfter.sourceFingerprint).toBe(assetBefore.sourceFingerprint);
            expect(output).toMatch(/optimized=\d*[1-9]\d*/);
        } finally {
            writeFileSync(CACHE_PATH, JSON.stringify(cacheBefore, null, 2) + "\n");
        }
    });
